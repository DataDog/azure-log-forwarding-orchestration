...
az deployment mg create --management-group-id "Azure-Integrations-Mg" --location eastus --name lfoqaenv --template-file ./deploy/azuredeploy.bicep --parameters monitoredSubscriptions="[\"$AZURE_SUBSCRIPTION_ID\"]" --parameters controlPlaneLocation=eastus --parameters controlPlaneSubscriptionId="$AZURE_SUBSCRIPTION_ID" --parameters controlPlaneResourceGroupName=lfoqaenv --parameters datadogApiKey="$DD_API_KEY" --parameters datadogSite=datadoghq.com --parameters datadogTelemetry=true --parameters imageRegistry=lfoqa.azurecr.io
