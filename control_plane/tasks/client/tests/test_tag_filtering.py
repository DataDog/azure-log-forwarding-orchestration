...
class TestTagFiltering(TestCase):
    def test_resource_tag_filtering_empty_tags_and_filters(self):
        tag_filters = []
        resource_tags = []
    ...
    def test_resource_tag_filtering_single_inclusive_match(self):
        tag_filters = ["datadog:true"]
        resource_tags = ["datadog:true"]
        self.assertTrue(parse_filtering_rule(tag_filters)(resource_tags))

    def test_resource_tag_filtering_single_inclusive_nonmatch(self):
        tag_filters = ["datadog:true"]
        resource_tags = []
        self.assertFalse(parse_filtering_rule(tag_filters)(resource_tags))

    def test_resource_tag_filtering_single_excluding_match(self):
        tag_filters = ["!major:fomo"]
        resource_tags = ["major:fomo"]
        self.assertFalse(parse_filtering_rule(tag_filters)(resource_tags))

    def test_resource_tag_filtering_single_excluding_nonmatch(self):
        tag_filters = ["!major:fomo"]
    ...
    def test_resource_tag_filtering_both_list_given(self):
        ...
        resource_tags = ["datadog:true", "env:test", "happy:days", "major:fomo", "good:bye", "bad:times"]
        self.assertFalse(parse_filtering_rule(tag_filters)(resource_tags))
