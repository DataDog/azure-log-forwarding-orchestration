...
async def run_parallel(
    ...
    return await gather(*[create_task(async_func(item)) async for item in async_iter], return_exceptions=True)


async def delete_setting(client: MonitorManagementClient, resource_id: str, setting: str):
    if setting and setting.startswith(DIAGNOSTIC_SETTING_PREFIX):
        # print(f"Deleting diagnostic setting {setting}")
        try:
            if not DRY_RUN:
                await client.diagnostic_settings.delete(resource_id, setting)
            print(f"Deleted setting {setting} for resource {resource_id}")
        except Exception:
...
async def process_resource(monitor_client: MonitorManagementClient, resource_id: str) -> list[BaseException]:
    # global resource_idx
    # print(f"Processing resource {resource_idx}/500ish")
    # resource_idx += 1
...
async def main():
    ...
    # subs: list[str] = [sub.subscription_id for sub in sub_client.subscriptions.list()]  # type: ignore
